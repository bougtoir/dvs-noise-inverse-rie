#!/usr/bin/env python3
"""
Generate the IEEE Sensors Journal manuscript (Word) for the DVS noise inverse problem.

Formatting:
- IEEE two-column journal style (US letter, 10 pt, single-spaced)
- Title/author block/abstract/keywords on first page
- Figures inline with captions below, also supplied as separate files
- Numbered references in IEEE order [1], [1,2]
...
OUT_DIR = SCRIPT_DIR
# Final, manuscript-number-aligned figures (produced by prepare_ieee_figures.py)
...
# =========================================================
# IEEE section/column layout helpers
# =========================================================
...
def _start_section(doc, columns: int):
    """Add a new continuous section and set its column count."""
...
# Citation renumbering in order of appearance (Vancouver/IEEE style).
CITE_REGISTRY = {}


CITATION_BR_RE = re.compile(r'\[(\d+(?:[-,–]\d+)*)\]')


def _parse_cite_content(content):
    """Expand bracketed citation content into a flat list of old integers."""
    nums = []
    last = None
    sep = None
    for part in re.split(r'([,\-–])', content):
        ...
        if part in {',', '-', '–'}:
            ...
            continue
        n = int(part)
        if sep in {'-', '–'} and last is not None and n > last:
            nums.extend(range(last + 1, n + 1))
        else:
            nums.append(n)
        last = n
        sep = None
...
def _cite_label(old):
    """Return the order-of-appearance label for an original reference number."""
    if old not in CITE_REGISTRY:
        CITE_REGISTRY[old] = len(CITE_REGISTRY) + 1
    return CITE_REGISTRY[old]


def _format_cite_list(new_nums):
    """Collapse sorted new citation labels into IEEE bracketed form."""
    nums = sorted(set(new_nums))
    if not nums:
        return '[]'
    parts = []
    start = end = nums[0]
...
def _renumber_bracket(match):
    nums = _parse_cite_content(match.group(1))
    new_nums = [_cite_label(n) for n in nums]
    return _format_cite_list(new_nums)


def _renumber_cite_text(text):
    """Rewrite all citation brackets in text to order-of-appearance numbers."""
    return CITATION_BR_RE.sub(_renumber_bracket, text)


def _renumber_reference(ref):
    """Replace the leading [old] in a reference entry with [new]; None if orphan."""
    m = CITATION_BR_RE.search(ref)
    if not m:
        return ref
    old = int(m.group(1))
    if old not in CITE_REGISTRY:
        return None
    new = _cite_label(old)
    return ref[:m.start()] + _format_cite_list([new]) + ref[m.end():]
...
def add_paragraph(doc, text, style='Normal', bold=False, italic=False,
    ...
    # Number citations in order of appearance, then replace metrics placeholders.
    text = _renumber_cite_text(text)
...
def add_figure(doc, img_path, caption, width=Inches(5.5)):
    caption = _renumber_cite_text(caption)
    ...
    for part in parts:
        if part:
            _add_run_for_part(p_cap, part, base_size=Pt(8))
    return p_cap
...
def add_figure_span(doc, img_path, caption, width=COLUMN_WIDTH):
    """Insert a column-width figure in the two-column body."""
    # Force all figures to the same column width for the IEEE two-column layout.
    return add_figure(doc, img_path, caption, width=COLUMN_WIDTH)
...
def load_results():
    ...
    return {
        ...
        'a5_max_snr': f"{summary['a5_simulation']['max_improvement']:.2f}",
        # Demo
...
def build_manuscript():
    ...
    add_figure_span(doc, FIG_DIR / 'fig1_schematic.png',
    ...
    add_paragraph(doc, (
        ...
        'sources) are discussed in Sections 3.3 and 7.1.'
    ))

    # Fig 2: Optimal rho*
    add_figure_span(doc, FIG_DIR / 'fig2_optimal_rho.png',
    ...
    # Fig 3: Pipeline
    add_figure_span(doc, FIG_DIR / 'fig3_pipeline_en.png',
    ...
    # Fig 4: SR curves
    add_figure_span(doc, FIG_DIR / 'fig4_sr_curves.png',
    ...
    add_figure_span(doc, FIG_DIR / 'fig5_detection_and_roc.png',
    ...
    add_figure_span(doc, FIG_DIR / 'fig7_evaluation_and_application.png',
               'Fig. 6. Experimental evaluation on EBSSA and its interpretation '
    ...
    add_figure_span(doc, FIG_DIR / 'fig8_proof_of_concept.png',
               'Fig. 7. Event-based proof-of-concept on EBSSA Recording #0. '
    ...
    add_paragraph(doc, (
        'The Fano factor spatial map (Fig. 7e) shows clear separation between '
    ...
    add_heading(doc, '7.1. Limitations and future work', level=2)
    add_paragraph(doc, (
        ...
        '(v) Hardware and dataset extension: validation on SciDVS [24] hardware and '
        'the DVSNOISE20 [25] benchmark is planned. '
    ...
    conclusions = [
        ...
        '(up to {a5_max_snr}\u00d7) improvement\u2014consistent with the theoretical excess-noise-regime prediction.',
    ...
    add_paragraph(doc, (
        ...
        'mean {a5_mean_snr}\u00d7 SNR gain (up to {a5_max_snr}\u00d7) corresponding to approximately 2.5 magnitudes '
    ...
    add_paragraph(doc, (
        ...
        'The EBSSA data are available through the Tonic library [7] and all code, '
        'figures, and submission artifacts are mirrored at '
        'https://github.com/bougtoir/dvs-noise-inverse-ieee-sensors.'
    ), italic=False)

    # =========================================================
    # References (IEEE numbered citation order)
    # =========================================================
    ...
    references = [
        ...
        "[24] R. Graça, S. Zhou, B. McReynolds, T. Delbruck, SciDVS: A scientific event camera with 1.7% temporal contrast sensitivity at 0.7 lux, ESSERC 2024, DOI:10.1109/esserc62670.2024.10719521.",
        "[25] R.W. Baldwin, M. Almatrafi, V. Asari, K. Hirakawa, Event probability mask (EPM) and event denoising convolutional neural network (EDnCNN), Proc. CVPR, 2020.",
    ]
    ordered_refs = [''] * len(CITE_REGISTRY)
    for ref in references:
        renumbered = _renumber_reference(ref)
        if renumbered is None:
            continue
        new_num = int(CITATION_BR_RE.search(renumbered).group(1))
        ordered_refs[new_num - 1] = renumbered
    for ref in ordered_refs:
        add_reference(doc, ref, space_after=3)

    # Save
    out_path = OUT_DIR / 'manuscript_ieee_sensors.docx'
